<template>
  <div class="card-wrapper van-hairline--bottom">
    ...
    <div class="card-content">
      ...
      <div class="counter">
        <div @touchend="counter(id, -1)" v-if="num && selshow">
          <img src="https://duyi-bucket.oss-cn-beijing.aliyuncs.com/img/rec.png" />
        </div>
        <div class="num">{{ num }}</div>
        <div @touchend="counter(id, 1, inventory)" v-if="selshow">
...
export default {
  props: ['images', 'tags', 'title', 'price', 'desc', 'id', 'num', 'nofly', 'selshow', 'inventory'],
  methods: {
    ...mapMutations(['storageChange', 'purStorageChange']),
    counter(id, num) {
      this.storageChange({ id, value: num, inventory: this.inventory });
      if (num === -1) {
        return;
      }
      if (this.$store.state.buytoomuch) {
        Toast('没库存了');
        return;
      }
      if (this.nofly) {
